...
const Attributes: React.FC = () => {
    ...
    const deleteMutation = useMutation({
        ...
        onError: (error: any) => {
            const errorMessage = error.response?.data?.message || "Error al eliminar el atributo. Podría estar en uso.";
    ...
    if (error) {
        ...
        if (message.includes('403')) {
            errorMessage = "Acceso Denegado (403): No tienes permiso para ver los atributos.";
        }
        return (
          <Card className="p-6">
                ...
                <CardContent className="p-0">
                    {isLoading ? (
                        <div className="flex items-center justify-center p-8 text-sm text-gray-500">
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Cargando lista de atributos...
                        </div>
                        ...
                        <div className="text-center p-8 space-y-2">
                            ...
                            {attributes.length === 0 && (
                                <Button onClick={() => handleOpenForm(null)} variant="link" className="text-purple-600">
                        ...
                        <Table>
                            <TableHeader className="dark:bg-gray-800">
                                <TableRow>
                                    <TableHead className="text-gray-600 dark:text-gray-400">Nombre del Atributo</TableHead>
                                    <TableHead className="text-gray-600 dark:text-gray-400">Tipo de Dato</TableHead>
                                    <TableHead className="text-center w-[120px] text-gray-600 dark:text-gray-400">Acciones</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody className="dark:bg-gray-900">
                                {filteredAttributes.map((attr) => (
                                    <TableRow key={attr.id} className="dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/70">
                                        <TableCell className="font-semibold text-gray-900 dark:text-gray-100">{attr.name}</TableCell>
                                        <TableCell className="font-mono text-sm text-gray-600 dark:text-gray-300">{attr.dataType}</TableCell>
            ...
            <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
                <DialogContent className="sm:max-w-md dark:bg-gray-900">
                    ...
                    <p className="text-gray-700 dark:text-gray-300 py-4">
                        ¿Estás seguro de que deseas eliminar el atributo **{attributeToDeleteName}**?
                    </p>
