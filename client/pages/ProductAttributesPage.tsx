...
import { 
    ArrowLeft, Plus, Trash2, Save, X, Tag, AlertCircle, Loader2, 
    Box, Pencil, Search
} from 'lucide-react';
...
const ProductAttributesPage: React.FC = () => {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['product-attribute-values', productId] });
            toast.success('Atributo añadido');
            handleCloseModal();
        },
        onError: (err: any) => toast.error(err.response?.data?.message || 'Error al añadir')
    });

    const updateMutation = useMutation({
        mutationFn: (data: { attrId: number, value: string }) => 
            updateProductAttributeValue(productId, data.attrId, { value: data.value }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['product-attribute-values', productId] });
    ...
    const deleteMutation = useMutation({
        mutationFn: (attrId: number) => deleteProductAttributeValue(productId, attrId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['product-attribute-values', productId] });
    ...
    const handleSubmit = () => {
        ...
        if (editingValue) {
            updateMutation.mutate({ attrId: editingValue.attributeId, value: valueInput });
        } else {
            createMutation.mutate({ productId, attributeId: Number(selectedAttributeId), value: valueInput });
        }
    };
    ...
    const availableAttributes = allAttributes.filter(a => 
        !productValues.some(pv => pv.attributeId === a.id)
    );
    ...
    const filteredValues = productValues.filter(val => {
        ...
        return def?.name.toLowerCase().includes(term) || val.value.toLowerCase().includes(term);
    });

    if (isProductLoading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin w-8 h-8 text-emerald-600"/></div>;
        ...
        <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900 p-6">
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <Button onClick={() => { setEditingValue(null); setSelectedAttributeId(''); setValueInput(''); setIsModalOpen(true); }} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm">
                ...
                <div className="grid gap-6">
                    ...
                    {isValuesLoading ? (
                        ...
                        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Sin atributos asignados</h3>
                            <p className="text-gray-500 text-sm mt-1">Este producto no tiene características especiales configuradas.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {filteredValues.map((val) => {
                                const def = getAttrDef(val.attributeId);
                                return (
                                    <Card key={val.id} className="group hover:shadow-md transition-shadow dark:bg-gray-800 border-l-4 border-l-emerald-500">
                                        <CardContent className="p-4 flex justify-between items-start">
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <Badge variant="secondary" className="text-[10px] px-1.5 h-5 bg-gray-100 dark:bg-gray-700 text-gray-500">
                                                        {def?.dataType}
                                                    </Badge>
                                                </div>
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <Button 
                                                    ...
                                                    onClick={() => {
                                                        if(confirm('¿Eliminar atributo?')) deleteMutation.mutate(val.attributeId);
                                                    }}
                                                >
            ...
            <Dialog open={isModalOpen} onOpenChange={handleCloseModal}>
                <DialogContent className="sm:max-w-[400px]">
                    <DialogHeader>
                        <DialogTitle>{editingValue ? 'Editar Valor' : 'Asignar Atributo'}</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    {editingValue ? (
                                        ...
                                        [getAttrDef(editingValue.attributeId)].map(a => a && (
                                ...
                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                                    ...
                                    {def.dataType === 'Boolean' ? (
                                        ...
                                        <Input 
                                            ...
                                            placeholder={def.dataType === 'Number' ? "0.00" : "Escriba el valor..."}
                                            value={valueInput}
                        ...
                        <div className="flex justify-end gap-2 mt-4 pt-4 border-t">
                            ...
                            <Button 
                                ...
                                disabled={!selectedAttributeId || !valueInput || createMutation.isPending || updateMutation.isPending}
