...
const ProductList: React.FC = () => {
  ...
  const [productDetail, setProductDetail] = useState<ProductListItem | null>(
    null,
  );
  ...
  const productListWithStock: ProductListWithStock[] = useMemo(() => {
    return products.map((product, index) => {
      ...
      const currentStock =
        stockQueryResult?.data !== undefined ? stockQueryResult.data : 0;
  ...
  const filteredProducts = productListWithStock.filter(
    (p) =>
  ...
  if (isLoadingProducts) {
    return (
      <div className="p-6 flex justify-center items-center h-64 dark:text-gray-300">
        <Loader2 className="w-8 h-8 animate-spin text-emerald-600" />{" "}
        <span className="ml-3 text-lg">Cargando catálogo...</span>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="p-6 bg-red-100 text-red-800 rounded-lg border border-red-400">
    ...
    <div className="p-6 space-y-6 bg-gray-50 dark:bg-gray-800 min-h-screen">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl max-w-sm w-full p-6 space-y-4">
            ...
            <p className="text-gray-700 dark:text-gray-300">
              ...
              {productToDeleteName}** (ID: {productToDeleteId})? Esta acción es
              irreversible.
            </p>
      ...
      <div className="flex justify-between items-center pb-4">
        ...
        <div className="flex space-x-4">
          ...
          <Button
            variant="outline"
            onClick={() => navigate("/inventory/products/bulk-attributes")}
            className="flex items-center space-x-2 border-purple-200 text-purple-700 hover:bg-purple-50 dark:border-purple-800 dark:text-purple-400 dark:hover:bg-purple-900/20"
            title="Ir a herramienta de etiquetado masivo"
          >
            <Tags className="w-5 h-5" />
            <span className="hidden sm:inline">Etiquetado Masivo</span>
          </Button>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-100 dark:bg-gray-800 hover:bg-gray-100/80 dark:hover:bg-gray-700/80">
              ...
              <TableHead className="text-center w-[150px] text-gray-600 dark:text-gray-400">
                Acciones
              </TableHead>{" "}
              {}
            </TableRow>
          ...
          <TableBody>
            {isStockLoading ? (
              <TableRow>
                <TableCell
                  colSpan={10}
                  className="text-center py-10 text-base text-indigo-500 dark:text-indigo-400"
                >
                  <Loader2 className="h-5 w-5 mr-3 inline-block animate-spin" />{" "}
                  Calculando existencias...
                </TableCell>
              </TableRow>
              ...
              filteredProducts.map((product) => (
                <TableRow
                  key={product.id}
                  className="border-gray-200 dark:border-gray-800 hover:bg-emerald-50/20 dark:hover:bg-gray-800 transition-colors duration-150"
                >
                  <TableCell>
                    <img
                      ...
                      onError={(e) => {
                        ...
                        target.onerror = null;
                        target.src =
                          "https://placehold.co/40x60/ccc/333?text=N/A";
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <div className="flex justify-center space-x-1">
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() =>
                          product.id && handleOpenDetailModal(product)
                        }
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() =>
                          product.id && handleConfirmDelete(product)
                        }
                      ...
                      <Button
                        ...
                        onClick={() =>
                          navigate(
                            `/inventory/products/${product.id}/attributes`,
                          )
                        }
