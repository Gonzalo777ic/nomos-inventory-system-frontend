...
const CollectionDetailPage: React.FC = () => {
    ...
    const { data: ar, isLoading, error, refetch } = useQuery<AccountsReceivable>({
    ...
    const paidAmount = ar.installments.reduce((sum, i) => sum + (i.paidAmount || 0), 0);
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Asociada a Venta #{ar.sale?.id} • {new Date(ar.sale?.saleDate || "").toLocaleDateString()}
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">${ar.totalAmount.toFixed(2)}</div>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Abonado</CardTitle>
                    </CardHeader>
                    <CardContent>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {ar.installments.map((inst) => {
                                        const pending = inst.expectedAmount - (inst.paidAmount || 0);
                                        const isLate = inst.status !== 'PAID' && new Date(inst.dueDate) < new Date();
                                        
                                        return (
                                            <TableRow key={inst.id}>
                                                <TableCell className="font-medium">{inst.number}</TableCell>
                                                <TableCell>
                                                    <div className={`flex items-center gap-2 ${isLate ? "text-red-600 font-semibold" : ""}`}>
                                                        {new Date(inst.dueDate).toLocaleDateString()}
                                                        {isLate && <AlertCircle className="w-3 h-3" />}
                                                    </div>
                                                </TableCell>
                                                <TableCell className="text-right">${inst.expectedAmount.toFixed(2)}</TableCell>
                                    ...
                                    <TableBody>
                                        {ar.collections.map((col) => (
                                            <TableRow key={col.id} className={col.status === 'ANULADO' ? "opacity-50" : ""}>
                                                ...
                                                <TableCell className="text-right font-bold">${col.amount.toFixed(2)}</TableCell>
                                                <TableCell className="text-center">
                                                    {col.status === 'ANULADO' ? (
