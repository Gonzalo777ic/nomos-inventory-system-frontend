...
export const AttributeProductsSheet: React.FC<Props> = ({
  attributeId,
  attributeName,
  isOpen,
  onClose,
      ...
      <SheetContent className="w-full sm:w-[540px] flex flex-col h-full bg-white dark:bg-gray-900 border-l dark:border-gray-800">
        <SheetHeader className="pb-4 border-b dark:border-gray-800">
          ...
          <SheetDescription className="dark:text-gray-400">
            Listado de productos que tienen configurado este atributo
            actualmente.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 py-4 overflow-hidden relative">
          {isLoading ? (
            ...
            <div className="flex flex-col justify-center items-center h-full text-gray-400 gap-2 text-center p-8">
              ...
              <p>
                Ningún producto utiliza el atributo{" "}
                <strong>{attributeName}</strong> todavía.
              </p>
            </div>
          ) : (
                ...
                {products.map((item) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <Badge
                        variant="outline"
                        className="bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800 font-mono"
                      >
                        {item.attributeValue}
                      </Badge>
                    </div>
        ...
        <div className="pt-4 border-t dark:border-gray-800 text-xs text-center text-gray-400">
          Total: {products.length} productos asociados
        </div>
      </SheetContent>
    </Sheet>
  );
